...
import { samsungProductRoutes } from "@/shared/routes/index-routes";
...
function SamsungProductsSection() {
  const [data, setData] = React.useState<any>(null);

  React.useEffect(() => {
    const samsung = samsungProductRoutes[0].onFetch(
      samsungProductRoutes[0].slug
    );
    samsung.then((data) => {
      console.log(data);
      setData(data.data);
      ...
      <div className="container mx-auto md:px-16 2xl:px-32 px-4">
        <header className="flex flex-col justify-center items-center gap-y-2 w-full pb-4">
          ...
          <p className="text-sm md:text-lg 2xl:text-xl text-slate-500">
            Shop Samsung products and pick what best for you
          </p>
        </header>
        <div className="md:hidden flex flex-col w-full h-full gap-y-4">
          ...
          <div className="min-w-full flex flex-wrap gap-4 justify-center items-center py-8">
            {samsungProductRoutes.map((route) => (
              <Button
                key={route.id}
          ...
          <div className="w-full flex justify-center items-center py-8">
            <SectionButton link={URLS.samsung} />
          </div>
        </div>
        <div className="hidden md:flex flex-col w-full h-full gap-y-4">
          <Tabs defaultValue="galaxy-mobile" className="w-full">
